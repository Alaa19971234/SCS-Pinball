...
export var flipper_speed = 10  # adjust the speed of the flipper rotation

var flipper_rot = -0.5  # keep track of the current flipper rotation
...
func _input(event):
		if event.is_action_pressed("flipper"):
			...
			flipper_rot = -0.5  # set the flipper rotation back to 0 to stop the rotation
